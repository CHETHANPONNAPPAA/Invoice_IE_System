...
# working directory. Streamlit Cloud (and some other hosts) run the app
# from the repo root, which is NOT necessarily the folder app.py lives in
# if your repo nests it in a subfolder -- bare relative paths like
# "sample_data/invoice_1.txt" then silently point at the wrong place and
# raise FileNotFoundError. Resolving against __file__ makes this work no
# matter where the process's cwd ends up.
...
SAMPLE_FILES = {
    "Sample invoice 1 (Acme → Beta Retail)": BASE_DIR / "sample_data" / "invoice_1.txt",
    "Sample invoice 2 (Global Tech → Nova Electronics)": BASE_DIR / "sample_data" / "invoice_2.txt",
}
...
with tabs[6]:
    ...
    try:
        with open(BASE_DIR / "report.md", encoding="utf-8") as f:
            st.markdown(f.read())
